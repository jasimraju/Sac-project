<template>
    <div class="modal fade" id="authentication"  aria-labelledby="exampleModalLabel" aria-hidden="true">
        ...
        <p>It may take a minute to receive your code.<br/> Haven’t received? <span class="nav-link" @click ="resend_otp" >Resend a new code.</span></p>
        </div>
    ...
    export default {
        ...
        data() {
            ...
            methods: {
                resend_otp:function(){
                    var postdata = {};
                    this.globalfileupload_oneregistration('user/generate/otp?mobileNo='+this.phone_number, postdata).then((result) => {
            ...
            handleOtp() {
                ...
                if (checkvalidtion) {
                    ...
                    this.globalPostreauth_onetime('user/verify/otp?mobileNo='+this.phone_number+'&otp='+otp_valu).then((result) => {
                        console.log(result);									
                        ...
                        this.errors.push('Otp You Have Entered is Incorrect , please try again');
                        if (error.response) {
                            //do something
